...
function setCookie(cookieName, cookieValue, expireDays = 0) {
    ...
    // Gibt uns die nötigen Millisekunden an um das Entsprechende Datum zu generieren
    const expireDate = new Date().setTime(currentDate + (expireDays*24*60*60*1000));

    // Erstellt das Datum und fügt es in den String hinzu
...
function deleteCookie(cookieName) {
    ...
    document.cookie = `${cookieName}=; expiress=${new Date(0).toUTCString()}`;
}
...
// Versteckt das Cookie "banner" falls der cookie bereits existiert
function showBanner() {
    const cookies = document.cookie;
...
// Pro Verbindung, d.h http und https Seiten zwei getrennte Storages
// Maximale Größe: BS und Browser abhängig => HTML5 Spezifikation (5MB)
